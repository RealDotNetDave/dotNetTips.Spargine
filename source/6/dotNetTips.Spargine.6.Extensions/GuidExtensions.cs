...
namespace DotNetTips.Spargine.Extensions
{
	...
	public static class GuidExtensions
	{
		...
		public static bool FastEquals(this in Guid left, in Guid right)
		{
			...
			if (Sse2.IsSupported)
			{
				Vector128<byte> leftVector = Unsafe.ReadUnaligned<Vector128<byte>>(ref Unsafe.As<Guid, byte>(ref Unsafe.AsRef(in left)));
				Vector128<byte> rightVector = Unsafe.ReadUnaligned<Vector128<byte>>(ref Unsafe.As<Guid, byte>(ref Unsafe.AsRef(in right)));

				Vector128<byte> equals = Sse2.CompareEqual(leftVector, rightVector);
